...
makeKakaha = function(kakaha, images, desc)
{
    var id = $(kakaha).parent().parent().find(".ananimas").text();
    var kakahaNum = Math.abs(id.hashCode()) % images.length;
    $(kakaha).attr({
        'src': images[kakahaNum],
        'title': desc}); 
}

sortaGovna = function(selector, images, desc)
{
    for (i in selector)
        $('img[title="' + selector[i] + '"]').each(
            function()
            {
                makeKakaha(this, images, desc);
            });
}
